 ...
 * Description: Custom WooCommerce plugin for managing products via My Account page.
 * Version: 1.0.0
...
class WP_My_Product_Webspark
{
    ...
    private function check_wc_dependency()
    {
        if (!class_exists('WooCommerce')) {
            add_action('admin_notices', function () {
                echo '<div class="error"><p><strong>WP My Product Webspark</strong> requires WooCommerce to be installed and active.</p></div>';
            });
            return false;
        }
        return true;
    ...
    public function handle_add_product_submission()
    {
        ...
        if ($product_id) {
            ...
            if ($current_author != $user_id) {
                wc_add_notice(__('You are not allowed to edit this product.', 'wp-my-product-webspark'), 'error');
                return;
            }
        ...
        if (is_wp_error($product_id)) {
            wc_add_notice(__('There was an error saving the product.', 'wp-my-product-webspark'), 'error');
            return;
        }
    ...
    if (in_array($file_type['type'], $allowed_types)) {
        ...
        if (is_wp_error($uploaded)) {
            wc_add_notice(__('Error uploading the image: ', 'wp-my-product-webspark') . $uploaded->get_error_message(), 'error');
            return; // Зупиняємо створення товару
        } else {
            set_post_thumbnail($product_id, $uploaded);
        }
    } else {
        wc_add_notice(__('Invalid file type. Please upload an image.', 'wp-my-product-webspark'), 'error');
        return; // Зупиняємо створення товару
    }
}
        ...
        wc_add_notice(__('Product successfully saved!', 'wp-my-product-webspark'), 'success');
    ...
    public function send_admin_email($post_id, $post)
    {
        ...
        $message = sprintf(
            ...
            __('Product Name:', 'wp-my-product-webspark') . " " . $post->post_title,
            __('Author Page:', 'wp-my-product-webspark') . " <a href='{$author_url}'>{$author_url}</a>",
            __('Edit Product:', 'wp-my-product-webspark') . " <a href='{$edit_product_url}'>{$edit_product_url}</a>"
        );
